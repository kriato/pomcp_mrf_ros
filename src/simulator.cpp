    ...
    const STATUS& status) const
{
    ...
    if (Knowledge.RolloutLevel >= KNOWLEDGE::SMART)
    {
        ...
        if (!actions.empty())
        {
                ...
                return actions[RND->Random(actions.size())];
            #else
    ...
    VNODE* vnode, const STATUS& status) const
{
    ...
    {
        vnode->SetChildren(+pLargeInteger, -pInfinity);
    }
...
void SIMULATOR::ResetAgent() const
{

}

int SIMULATOR::GetRelKnowLevel() const
{
    return Knowledge.relKnowLevel;
}

void SIMULATOR::DisplayBeliefDistribution(const BELIEF_STATE& beliefState,
    ostream& ostr) const
{
}

void SIMULATOR::GetBeliefDistribution(const BELIEF_STATE &beliefState, std::vector<std::pair<int, int>> &beliefs) const
{
}

vector<double*>* SIMULATOR::CreateStateRelKnowledge(const STATE& state, int nConnComp, double relProbab) const
{
    return 0;
}


STATE* SIMULATOR::CreateStartState(std::vector<STATE*>* allParticles, std::vector<double> allParticleCumProbs, int id) const
{
    return 0;
}
...
void SIMULATOR::PropagateChange(STATE& state, int changedVariableIndex, std::vector<double*>* stateVarRelationships,
         vector<int>* alreadyExploredVarIndices) const
{
}

void SIMULATOR::PropagateChangeToConnectedComponent(STATE& state, int changedVariableIndex, int newVal,
        std::vector<double*>* stateVarRelationships) const
{
}

void SIMULATOR::Id2State(const int id, std::vector<int> &state) const
{
}

void SIMULATOR::writeHistory(std::string what, bool append) const
{

}

vector<double*> SIMULATOR::FindUnexploredAdjacentVariableIndices(int currentVarIndex,
...
double SIMULATOR::ComputeParticleProbability(STATE& particle, std::vector<double*>* stateVarRelationships) const
{
    return 0.0;
}